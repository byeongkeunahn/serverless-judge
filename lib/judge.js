...
window.judgeSolution = async function judgeSolution(language, src, manifest) {
  ...
  const solver = createRunner(manifest.solver.language, solver_src, null);
  const challenger = createRunner(language, src, null);
  for (const test of manifest.tests) {
    if (test.type == "static") {
      ...
      for (var i = 0; i < test.max_try_count; i++) {
        const input_str = await window.runSolution(test.language, generator, encode_utf8(i.toString()));
        const result = await judgeSolutionSingle(solver, challenger, input_str);
        if (!result.verdict) {
          return result;
        }
